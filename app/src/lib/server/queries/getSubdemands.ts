...
const getSubdemands = async ({ nodeId }: { nodeId: string }) => {
	...
	if (!subDemandsResponse.ok) {
		error(subDemandsResponse.status, {
			message: `Subdemands response status: ${subDemandsResponse.status}`
		});
	}

	return subDemandsResponse.json().then(({ data: subDemands }) =>
		subDemands.map(({ attributes }) => ({
