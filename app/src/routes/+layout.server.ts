...
export const load: PageLoad = async ({}) => {
	...
	const nodesURL = `${DRUPAL_API_BASE_URL}${DRUPAL_NODES_PATH}?filter[field_microsite.id]=${DRUPAL_MICROSITE_UUID}`;
	...
	data.forEach(({ id, attributes, relationships }) => {
		const processedNode = {
			id,
			pageType: pageTypes[relationships[DRUPAL_PAGE_TYPE_FIELD].data.meta.drupal_internal__target_id],
		...
		switch(processedNode.pageType){
			...
			case 'About':
				...
				break;
			default:
				console.warn(`Page type ${relationships[DRUPAL_PAGE_TYPE_FIELD].data.meta.drupal_internal__target_id} is not supported.`)
		}
	});
